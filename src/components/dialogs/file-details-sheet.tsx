          ...
          <SheetHeader className="mb-4 p-0">
            <div className="flex items-center justify-between">
              <div className="flex flex-col space-y-1">
                ...
                <SheetDescription>
                  {selectedFile?.filename || metadata?.unc_path || metadata?.file_path || "Detailed information about the selected file"}
                </SheetDescription>
              </div>
              <SheetClose asChild>
                      ...
                      <dl className="grid grid-cols-1 gap-y-3 text-sm sm:grid-cols-2 lg:grid-cols-3 sm:gap-x-6">
                        ...
                        {metadata.indexed_at && (
                          <div>
                            ...
                            <dd className="p-1"><pre className="mt-1 max-h-80 overflow-auto rounded bg-muted p-2 text-xs">{new Date(metadata.indexed_at).toLocaleString()}</pre></dd>
                          </div>
                        )}
                ...
                <Card className="bg-muted/20">
                  <CardContent className="p-4 space-y-4">
                    {!contentVisibilityEnabled ? (
                      <p className="text-sm text-muted-foreground">Content visibility is disabled in settings.</p>
                    ) : contentLoading ? (
                      <div className="flex justify-center py-8">
                        <Spinner className="size-8" />
                      </div>
